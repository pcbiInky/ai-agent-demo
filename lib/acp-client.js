/**
 * 通用 ACP（Agent Client Protocol）客户端
 *
 * dsh 与 kimi 都通过 ACP over NDJSON 提供长期运行的 JSON-RPC 服务：
 *   dsh --profile acp
 *   kimi acp
 * 二者的消息格式一致，因此共用这一层双向驱动；各 CLI 只需在自己的
 * 包装模块里声明启动参数（见 dsh-acp-client.js / kimi-acp-client.js）。
 *
 * 一轮调用的生命周期：
 ...
 *   （invoke.js 中 ACP 类 CLI 的 supportsSystemPrompt=false 已走该回退）。
 */
...
/**
 * 把角色配置里的模型名解析成 ACP agent 广告的 select value。
 * dsh 的模型 value 是 JSON 元组字符串，例如 '["deepseek-official","deepseek-v4-flash"]'；
 * kimi 的模型 value 是别名，例如 "kimi-code/k3"，name 是 "K3"。
 * 角色配置里写 value、name 或元组成员都能匹配上，匹配不到则原样透传。
 *
...
/**
 * 启动一个 ACP agent 并完成一轮对话。
 *
 * @param {object} params
 * @param {string} params.command - ACP agent 可执行文件
 * @param {string[]} [params.args=[]] - ACP agent 启动参数（如 ["--profile","acp"] / ["acp"]）
...
function invokeAcp({
  ...
  if (!command) {
    return Promise.reject(new Error("invokeAcp 缺少 command 参数"));
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    function handleNotification(message) {
      ...
      if (update.sessionUpdate === "usage_update") {
        // dsh 会随回复推送上下文用量：{ used, size }（已用 token / 窗口大小）
        emit({
          type: "usage_update",
          sessionId: message.params?.sessionId ?? null,
    ...
    function onAbort() {
      cancelAndReject(Object.assign(new Error(`${command} invoke 已中止`), { aborted: true }));
    }
    ...
    function cleanup(error) {
      ...
      for (const entry of pending.values()) {
        entry.reject(error || new Error("ACP 连接已关闭"));
      }
      pending.clear();
