...
const { buildUsageMetrics, windowsFromLegacy } = require("./usage-metrics");

const CLAUDE_SETTINGS_PATH = path.join(os.homedir(), ".claude", "settings.json");
const MINIMAXI_API_URL = "https://www.minimaxi.com/v1/api/openplatform/coding_plan/remains";

// claude 的额度维度与 codex 一致：5h + week
const CLAUDE_WINDOW_LABELS = { primaryLabel: "5h", secondaryLabel: "week" };
...
function normalizeMetrics(remain) {
  ...
  const primaryUsedPercent =
    remain.current_interval_total_count > 0
      ? Math.round((remain.current_interval_usage_count / remain.current_interval_total_count) * 100)
      : null;

  const secondaryUsedPercent =
    remain.current_weekly_total_count > 0
      ? Math.round((remain.current_weekly_usage_count / remain.current_weekly_total_count) * 100)
      : null;
