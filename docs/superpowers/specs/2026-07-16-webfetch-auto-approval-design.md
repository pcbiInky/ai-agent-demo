...
- 例外：域名经本机代理 DNS 解析到 `198.18.0.0/15` Fake-IP 时允许自动审批；直接请求该网段的 IP 字面量仍需人工审批。
...
4. 对域名使用 DNS 解析，只有全部 A/AAAA 结果均为公网地址或本机代理使用的 `198.18.0.0/15` Fake-IP 时才自动通过；DNS 失败或没有结果时不自动通过；
...
- 域名解析到 `198.18.0.0/15` Fake-IP 时自动通过，但该网段的 IP 字面量不自动通过；
